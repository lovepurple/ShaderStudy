...
public static partial class GeometryUtility
{
    ...
    /// <param name="isLineSegment"></param>
    /// <returns></returns>
    public static IntersectionResult RayToLineIntersection(Vector2 rayOrigin, Vector2 rayDirection, Vector2 lineStart, Vector2 lineEnd, bool isLineSegment)
    {
        IntersectionResult result = new IntersectionResult();
        ...
        if (rayDirection != lineDirection)
        {
            float d = Cross(rayDirection, lineDirection);

            if (d != 0)
            {
                Vector2 lineStartToRayOrigin = rayOrigin - lineStart;
                float r = Cross(lineDirection, lineStartToRayOrigin) / d;
                float s = Cross(rayDirection, lineStartToRayOrigin) / d;

                if (r >= 0)
                {
                    if ((isLineSegment && s >= 0 && s <= 1) || !isLineSegment)
                    {
                        Vector2 intersection = rayOrigin + r * rayDirection;
